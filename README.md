...
- [ ] `check_chip_identity`'s per-qubit fingerprinting (`_qubit_fingerprint_vector`) still reads local-only — same class of gap `get_alerts` had before the Turso work above, not yet converted (bigger rewrite, ~145 lines, cursor-based across multiple queries — flagged, not silently skipped)
