...
|---|---|
| **Tools** | 50 |
| **Providers** | IBM, IonQ (real hardware) + PennyLane, NVIDIA CUDA-Q (local simulation, no API key needed — see below) |
| **Tests** | 261 passed, 4 xfailed, 1 failed (live IonQ device-list API state at test time, unrelated to any code here — confirmed by re-running in isolation and checking git diff shows zero changes to the file it exercises) — full pass, 2026-09-19 |
...
**Experiment Memory + Intelligence** (`memory_summary`, `recommend_tolerance`) — IonQ's submit-job self-check logs every real prediction automatically (via `ionq_submit_job`); `verify_experiment`'s own path doesn't feed this yet, the same IonQ-only scope `verdict_track_record` already discloses in its own output. Once a real job completes, `ionq_sync_memory_for_job` records what actually happened next to it. `recommend_tolerance` uses that real accuracy history to recommend a tolerance for a given provider/device instead of a guessed default — honestly falling back to the default when there isn't yet enough real data to justify anything more specific. As of 2026-08-28 this is genuinely wired into the real decision path, not just computed and left unused: `amplification_tolerance` now defaults to `None` in `verify_experiment`, `ionq_submit_job`, and `ionq_preflight` — an explicit value from the caller is always used exactly as given, and only `None` triggers a real call to `recommend_tolerance`. Every result shows `tolerance_used`/`tolerance_source` (`"explicit"` / `"recommended (N data points)"` / `"default (not enough data)"`) so this is never a silent substitution. IonQ has real history behind this already (see above); IBM does not yet (that's real prediction-tracking parity work, scoped but not built — see `docs/ibm-prediction-tracking-scope-2026-08-28.md`), so an IBM call resolving `None` today will consistently fall back to the plain default until that lands — accumulating real IBM data points starts only from whenever that ships, nothing retroactive.
...
## PennyLane + NVIDIA CUDA-Q: classical work before real hardware, not "two more vendors"

Both are real, first-class providers now (`verify_experiment`/`falsify_claim`'s `provider` argument accepts `"pennylane"` and `"cudaq"` alongside `"ibm"`/`"ionq"`) — free, local, noiseless simulation, no API key or account needed for either. But the point isn't more places to run a circuit, it's using CUDA-Q's classical simulation and PennyLane's real gradients to do more honest work *classically*, on *any* circuit, before it's ever eligible for real hardware spend.

**Runs for everyone automatically, no signup, no key.** `cudaq` is a plain local library — CPU mode (`qpp-cpu`) works the moment it's installed; GPU mode (`nvidia`) activates automatically the instant `cudaq` itself detects it's running on a machine with a real NVIDIA GPU, physical or a rented cloud VM, no code change and no extra credentials required either way. Confirmed directly against this project's dev machine (macOS ARM64, no NVIDIA GPU): `cudaq.set_target("nvidia")` raises `RuntimeError: Invalid target name (nvidia)` — the macOS wheel doesn't ship that target at all — and target selection correctly, automatically falls back to `qpp-cpu`. Nobody is ever without this capability; a real GPU only makes it faster.

**`classical_reduction_check`** — wired into `verify_experiment`'s pipeline unconditionally, informational only (same "earn integration before it can block" pattern this project already used for `ground_truth_significance_test`/`register_mapping_check`). Deliberately does not reinvent two things this project already does well: for Clifford circuits, defers entirely to `core/stabilizer.py`'s exact, polynomial-time Gottesman-Knill tableau — confirmed directly that `cudaq` is never even invoked on that path, not just documented as such (a general-purpose simulator would be a strict regression there). For "strip the entangling gates and diff the result," that's already `falsify_claim`, not redone here with a different simulator underneath. What's genuinely new: an independent second exact-statevector implementation of `ideal_simulation`'s answer (confirmed empirically identical to Qiskit's own `Statevector` for the same circuit, so a real disagreement between them would mean an Aer-specific bug); and a circuit-intrinsic entanglement-entropy measurement of the real, unmodified circuit's exact final state — no hypothesis about which gate to strip needed, not binary/structural, a single continuous number answering "how much is this circuit doing quantum-mechanically, independent of any claim about it." Catches, for example, a badly-initialized variational circuit that ends up near a product state despite having real entangling gates present.

**`analyze_circuit_gradients`** — a genuinely different kind of check from anything else here: uses PennyLane's real autodiff (`qml.grad`) on a still-*parameterized* circuit definition, one lifecycle stage before a circuit is bound to concrete angles and turned into QASM (QASM is the final, fully-bound format — there's nothing left to differentiate once a circuit becomes QASM, so this can't live inside `verify()`'s pipeline the way the other checks do). Flags barren-plateau risk (near-zero gradient norm — confirmed with the real, reproducible effect: a random ansatz's gradient norm against a global observable dropped roughly 40x going from 2 to 12 qubits with the same random seed) and individual near-zero-gradient parameters (candidate redundant/cancelling gate pairs — confirmed directly with a literal `RX(θ)` then `RX(-θ)` pair, correctly flagged while a genuinely active parameter on the same circuit was not).

**`run_hybrid_reduction_loop`** — the real, iterative classical-quantum pattern published, real hybrid work (e.g. Deutsche Bahn/IQM's railway rescheduling) actually uses: a classical solver narrows/scores the problem, the QPU searches the smaller remainder, the real result feeds back to refine the next classical pass, repeat. Scoped honestly to where that pattern genuinely applies — Ising/QUBO-shaped optimization problems, not a claim that any arbitrary circuit can be automatically split into classical/quantum parts (that's a different, unsolved problem). Builds on this project's own existing LNAA-style oracle (`core/templates.py`'s `graph_coloring_oracle_circuit`) and its existing free classical verifier (`verify_graph_coloring`, O(edges) per candidate even though the search space is exponential) rather than rebuilding either — each round uses the real previous result to decide the next round's parameters (strengthening the RZZ phase-kick relative to the RX mixer, adding oracle depth if still far from a valid answer), confirmed directly: forcing a genuinely unsatisfiable instance (a triangle graph, which has no valid 2-coloring) shows the loop's parameters provably changing round over round based on the real failed result, not blind identical retrying. Returns a full round-by-round log, so this is auditable, not a black box. Sits alongside `run_graph_coloring_search` (a single round) rather than replacing it.

**`run_parallel_rail_search`** — generalizes a real, working, single-purpose tool from `quantum-hardware-mcp` (`run_parallel_collision_search`, IBM-only, with its circuit-building and job-tiling logic bundled together and duplicated a second time in a sibling function) into a reusable capability: the tiler (`core/parallel_rail_search.py`) knows nothing about what problem a rail is solving, only how to place/submit circuits and route each rail's counts back to its own decoder. Built around a real, confirmed architectural fact — IBM and IonQ don't tile the same way. IonQ already has native multi-circuit batching (a real list of separate circuits submitted as one job, sharing the $168.20 per-job floor across all of them, confirmed in this project's own billing code); IBM has none, so multiple rails get tiled into one combined circuit via qubit-offset `compose()` placement before a single submission — the same real slicing math `run_parallel_collision_search` already got right, confirmed directly against a real 2-rail run (a Bell pair + an isolated X, hand-verified to split back into the exact correct per-rail distributions) before being written into `providers/ibm.py`. PennyLane/cudaq run each rail locally in sequence, no cost floor to share there. One honest, real gap found while wiring this up: IonQ's free `ionq_simulator` target returns no async job at all, just a synchronous self-check with only the top-5 counts per circuit — every result decoded from that path is explicitly flagged `"approximate": true` rather than silently reporting an amplification computed from an incomplete distribution as if it were exact. Singmaster's collision search (`core/singmaster_search.py`, the classical Lucas-theorem collision math and LNAA circuit-building reused verbatim from `quantum-hardware-mcp`'s already-hardware-confirmed originals) is the first real plugin — confirmed against the project's own known, real collision, C(16,2) = C(10,3) = 120.
...
| `get_alerts` | Calibration drift alerts — spikes in CX error, readout error, T1, T2. Reads live from the shared Turso database (2026-08-30, see Roadmap) when configured, so this reflects real current state regardless of whose laptop last synced — not just whatever this local machine happens to have collected |
...
│   └── turso.py               # shared Turso database client — live device history,
│                                # used by providers/ibm.py's get_alerts/_recent_drift_alert
├── providers/
...
├── cloud_snapshot.py           # 2-hour GitHub Actions collector — device + qubit + pair
│                                # history to CSV and the shared Turso database
...
- [ ] `device_alerts` (stored cx/readout-error-spike and went-offline alerts, as opposed to the live T1/T2 check) still local-only — only written by the local collection path, not yet mirrored to Turso
